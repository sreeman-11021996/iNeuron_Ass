...
class Database:
    ...
    def __check_db (self,db_name):
        try:
            ...
            log.error('DataBase not found! - {e_val}'.format(e_val = e))
            return True
        else:
    ...
    def __check_table(self,table_name):
        try:
            ...
            log.warning("New Table - {e_val}".format(e_val = e))
            return True
        else:
            log.info("Table {table} already exists".format(table=table_name))
            return False
        ...
        if self.__check_table(table_name):
            if len(columns) == self.__COLUMNS_REQ:
                try:
                    ...
                    log.error("Could not create the table! - {e_val}".format(e_val = e))
                else:
    ...
    def __file_insert (self,table_name,file_insert):
        import csv
        try:
